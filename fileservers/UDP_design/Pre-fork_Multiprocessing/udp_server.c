...
printf("listening..\n");
*/
/* Creation of child processes- Pre-forking */
...
//for(;;)
//{
if(recvfrom(sock,buffer,2096, 0, (struct sockaddr *)&client, &length)<0)
{
...
/*bzero(data_client,2096);
if(recvfrom(sock,data_client,2096, 0, (struct sockaddr *)&client, &length)<0);// Connection establishment and creation if new socket.
//if (receive>0)
{
...
printf("the pid is %d\n", pid);
receive= recvfrom(sock, data_client, 2096,0,(struct sockaddr *)&client, &length);
...
data_write=sendto(sock,buffer,data_read,0,(struct sockaddr *)&client, length);
if(data_write<=0)
{
printf("File cannot be written\n");
}
